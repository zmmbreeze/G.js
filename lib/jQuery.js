...
/*jshint undef:true, browser:true, noarg:true, curly:true, regexp:true, newcap:true, trailing:true, noempty:true, regexp:false, evil:true, funcscope:true, iterator:false, loopfunc:true, multistr:true, boss:true, eqnull:true, eqeqeq:false, undef:false */
/*global G:true, jQuery:true, $:true */
...
define('jQuery', function() {
    ...
    /**
     * $(window).windowLoaded( callback )
     * 当已经load时直接执行。未load的时候加入load队列
     * @param {function} callback 回调函数
     */
    $.fn.windowLoaded = function(callback) {
        if (isWindowLoaded) {
            callback.call(this);
        } else {
            this.load(callback);
    ...
    /**
     * jQuery插件，使用此方法避免执行空对象
     *      $('.null').doOnce(function() {
     *          console.log(this); // jQuery object;
     *      });
     *      $('.null').doOnce(function( cookies ) {
     *          console.log( cookies );
     *      }, 'cookies');
     * @param {function} func 回调函数，this为此jquery对象
     * @param {string/array} modules 依赖的模块
     * @return {object} 此jquery对象
     */
    jQuery.fn.doOnce = function(func, modules) {
        var self = this;
        if (self.length) {
            // have modules
            if (modules) {
                G.req(modules, function() {
